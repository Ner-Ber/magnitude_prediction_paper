...
A second application may incorporating such a magnitude predictor into an earthquake rate forecasting model such as the epidemic type aftershock sequence model (ETAS) \neri{cite}. In ETAS, the rate of events is determined by the product of a set of functions. \neri{formalize?} Origin time of events of the forecasted sequence are then sampled from the rate function, and assigned with a magnitude by sampling from a fitted GR distribution. y replacing the randomly assigned magnitudes of ETAS with MAGNET or a similar magnitude predictor one will improve the predictive power of rate prediction. As we have shown - magnitudes predicted by MAGNET will estimate the true magnitudes in a higher likelihood.
